...
menu_principal() {
    while true; do
        ...
        echo -e "${AZ}==================================================${CL}"
        printf -e "${CY} 1)${CL} 🛠️ Configuraciones Iniciales del S.O (Server Base)"
        printf -e "${CY} 2)${CL} 🗄️ Bases de Datos (MariaDB / PostgreSQL)"
        printf -e "${CY} 3)${CL} 🌐 Servidores Web (Nginx / Apache)"
        printf -e "${CY} 4)${CL} 🐘 Gestor de PHP (Multi-versión)"
        printf -e "${CY} 5)${CL} 🔌 Gestor de Redes (Interfaces / PBR)"
        printf -e "${CY} 6)${CL} 💾 Gestor de Almacenamiento (LVM)"
        echo
